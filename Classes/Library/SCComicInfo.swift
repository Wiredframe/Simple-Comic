...
//  schema, which every tagger writes) into plain values. Parsed once when a comic is
//  scanned into the library and stored on the entry — no view ever touches XML.
//
//  Ported from the iOS Comic Reader app (Model/ComicInfo.swift); pure Foundation, so
//  the only change is the type prefix.
//
...
//  is worth a detail view. When the text doesn't match, `stories` is empty and callers
//  fall back to `summary` — nothing is lost, it just renders as a block instead of a list.
//
...
extension String {
	...
	/// whitespace-only ones freely, and " " is not metadata.
	var sc_nonEmpty: String? {
...
struct SCComicStory: Codable, Hashable, Identifiable {
	...
	/// verbatim rather than mapped to an enum: the label is localised by the tool that
	/// wrote it, and guessing at translations would only ever be wrong.
	let kind: String
	let title: String
...
/// The fields lifted out of ComicInfo.xml. All optional — a comic may carry none, some
/// or all of them.
struct SCComicInfoData {
	var series: String?
	var number: String?
	var title: String?
	var summary: String?
	...
	/// True when there's nothing worth showing. A file whose ComicInfo.xml holds only, say,
	/// a PageCount shouldn't light up the metadata section.
	var isEmpty: Bool {
...
enum SCComicInfoParser {
	...
	static func parse(_ data: Data) -> SCComicInfoData? {
		...
		// Treat those sentinels as absent so they never render as "-1".
	...
	/// Separates the roles on a credits line — `Disegni: X · Chine: Y`.
	private static let creditSeparator = CharacterSet(charactersIn: "·•")

	static func stories(fromSummary summary: String) -> [SCComicStory] {
		guard let entryPattern = entryPattern else { return [] }
		...
		for rawLine in summary.components(separatedBy: .newlines) {
			...
			if let match = entryPattern.firstMatch(in: line, range: range) {
				...
				stories.append(SCComicStory(number: number, kind: kind, title: title,
											code: code, credits: []))
				...
				stories.append(SCComicStory(number: last.number, kind: last.kind, title: last.title,
	...
	/// under its entry (how an index block is written) or list several roles at once.
	private static func credits(from line: String, indented: Bool) -> [SCComicStory.Credit]? {
		...
		var credits: [SCComicStory.Credit] = []
		for segment in segments {
			...
			credits.append(SCComicStory.Credit(role: role, name: name))
		}
		return credits
	}
	...
	private final class Delegate: NSObject, XMLParserDelegate {
		...
		func parser(_ parser: XMLParser, didEndElement element: String,
					namespaceURI: String?, qualifiedName: String?) {
			if depth == 2, let current = current {
